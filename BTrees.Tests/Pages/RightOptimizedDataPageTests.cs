...
{
    public sealed class RightOptimizedDataPageTests
    {
        [Fact]
        public void Empty_Page_Has_Correct_Count()
        {
            var page = RightOptimizedDataPage<DbInt32, DbInt32>.Empty();
            Assert.Equal(0, page.Count);
        }

        [Fact]
        public void Add_Tuple()
        {
            var page = RightOptimizedDataPage<DbInt32, DbInt32>.Empty();
            page.Add(1, 1);
        ...
        [InlineData(21)]
        [InlineData(24)]
        public void Add_Tuples(int count)
        {
            var page = RightOptimizedDataPage<DbInt32, DbInt32>.Empty();
        ...
        public void Read_Returns_Inserted_Element()
        {
            ...
            var page = RightOptimizedDataPage<DbInt32, DbText>.Empty();
            page.Add(1, expectedValue);
            var tuples = page.Read(1);

            Assert.Equal(1, tuples.Length);
            Assert.Equal(expectedValue, (string)tuples[0].Value, true);
        }
        ...
        [InlineData(4096)]
        [InlineData(8192)]
        public void Random_Key_Inserts(int count)
        {
            var values = RandomIntFactory.Generate(count).AsSpan();
            var keys = RandomIntFactory.Generate(count).AsSpan();
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                var value = values[i];
                dp.Add(key, value);
                Assert.True(dp.ContainsKey(key));
            }
        }
